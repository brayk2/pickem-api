"""
The shapes a token is written in and read back from.

These live below the components rather than inside `auth` because every route
in the application reads a DecodedToken, while `auth` itself needs LeagueService
to build the claims. With these in `auth`, that is a cycle: auth imports league,
league imports auth. Here, both point downwards at the same primitives.
"""
